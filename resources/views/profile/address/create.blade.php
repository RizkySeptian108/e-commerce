...
@section('main-page')
<div class="ms-10 me-10" x-data="addressForm">
    <x-title>Add New Address</x-title>
    <div class="mt-5 p-8 bg-white dark:bg-gray-800 shadow rounded-lg">
        <form method="post" action="{{ route('address.store') }}" class="flex flex-col gap-6">
            @csrf
            ...
            <div>
                ...
                <x-text-input id="address_label" name="address_label" type="text" class="mt-1 block w-full" value="{{ old('address_label') }}"  />
                <x-input-error :messages="$errors->get('address_label')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="address_benchmark" :value="__('Address Benchmark')" />
                <x-text-input id="address_benchmark" name="address_benchmark" type="text" class="mt-1 block w-full" value="{{ old('address_benchmark') }}" />
                <x-input-error :messages="$errors->get('address_benchmark')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="full_address" :value="__('Full Address')" />
                <x-textarea-input name="full_address" class="mt-1 block w-full" :cols="30" :rows="5" value="{{ old('full_address') }}" />
